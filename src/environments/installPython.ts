        ...
        commands.registerCommand('python.envManager.installPython', async () => {
            ...
            if (await fs.pathExists(MICROMAMBA_EXE)) {
                const message = [
                    ...
                    `If it does not worker, then initialize your shell using the command '${getDisplayPath(
                        MICROMAMBA_EXE,
                    )} shell init -s bash|zsh|cmd.exe|powershell|fish|xonsh -p ~/micromamba'.`,
                ];
            ...
            const detail = `Micromamba will downloaded into ${getDisplayPath(
                MICROMAMBA_DIR,
            )} \n& Shell scripts will be updated to put Micromamba into the current path.`;
            const selection = await window.showInformationMessage(
                'Do you want to download and setup Python via Micromamba?',
                { modal: true, detail },
                'Yes',
                moreInfo,
            );
            switch (selection) {
                case moreInfo:
                    void env.openExternal(
                        Uri.parse('https://mamba.readthedocs.io/en/latest/user_guide/micromamba.html'),
                    );
                    break;
                case 'Yes':
